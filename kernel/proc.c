...
int upreadsema(struct rwsemaphore *rws)
{
  ...
  if (--(rws->r)) // unlock when no more readers
    wakeup(rws);
...
void downwritesema(struct rwsemaphore *rws)
{
  ...
  rws->wq++; // incr write queue

  // wait until no other r/ws
  while (rws->r || rws->w)
    sleep(rws, &(rws->lk));

  // move from queue to writers
  rws->wq--;
  rws->w++;
